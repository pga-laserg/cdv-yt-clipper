...
async function main() {
    ...
    if (!workDirArg) {
        throw new Error('Usage: npm run test:ocr-v3 -- <workDir> [videoPath]');
    }
    ...
    const outPath = path.join(workDir, 'ocr.events.v3.json');
    const scriptPath = path.resolve(__dirname, 'pipeline/python/ocr_v3.py');
    ...
    if (!fs.existsSync(scriptPath)) throw new Error(`ocr_v3 script not found: ${scriptPath}`);
    ...
    if (progressSummary.events > 0) {
        console.log(
            `OCR v3 progress events=${progressSummary.events} first=${progressSummary.firstPercent?.toFixed(1)}% last=${progressSummary.lastPercent?.toFixed(1)}%`
        );
    }
    console.log(`Wrote OCR v3 events -> ${outPath} tracks=${tracks} segments=${segs}`);
}

main().catch((err) => {
    console.error('test-ocr-v3 failed:', err);
    process.exit(1);
});
